...
class UpdateDeliveryRequestScreen extends StatefulWidget {
  static const String routeName = '/update-delivery-request';
  final DeliveryRequest deliveryRequest;
  const UpdateDeliveryRequestScreen({
    Key? key, required this.deliveryRequest,
  }) : super(key: key);

  @override
  State<UpdateDeliveryRequestScreen> createState() =>
      _UpdateDeliveryRequestScreenState();
}

class _UpdateDeliveryRequestScreenState
    extends State<UpdateDeliveryRequestScreen> {
  ...
  final _updateDeliveryRequestFormKey = GlobalKey<FormState>();
...
*/
  ...
  @override
  void initState() {
    deliveryRequestTitleController.text = widget.deliveryRequest.title;
    descriptionController.text = widget.deliveryRequest.description;
    addressFromController.text = widget.deliveryRequest.addressFrom;
    addressToController.text = widget.deliveryRequest.addressTo;
    _dateTime = _dateTime;
    priceController.text = widget.deliveryRequest.price.toString();
    sizeController.text = widget.deliveryRequest.size;
    //images = images;
    super.initState();
  }

  void UpdateDeliveryRequest(DeliveryRequest deliveryRequest) {
    if (_updateDeliveryRequestFormKey.currentState!.validate() &&
        images.isNotEmpty) {
      senderServices.updateDeliveryRequest(
        context: context,
        deliveryRequest: deliveryRequest,
            ...
            //images: images,
             onSuccess: () { 
              showSnackBar( context, 'Delivery Request Modified Successfully ! ');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
          child: Form(
        key: _updateDeliveryRequestFormKey,
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              SimpleCustomTextField(
                ...
                hintText: widget.deliveryRequest.title,
              ),
              const SizedBox(
                height: 10,
              ),
